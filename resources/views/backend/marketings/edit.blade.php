<x-app-layout>
                ...
                <form action="{{ route('marketings.update', $marketing->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane active" id="basic-content" role="tabpanel" aria-labelledby="basic-tab">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <input type="text" class="input" id="name" name="name" value="{{ old('name', $marketing->name) }}" required />
                                </div>
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <input type="text" class="input" id="code" name="code" value="{{ old('code', $marketing->code) }}" required />
                                </div>
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <input type="number" class="input" id="due_date" name="due_date" value="{{ old('due_date', $marketing->due_date) }}" min="0" />
                                </div>
                                
                                <div class="flex flex-col gap-1">
                                    <label class="label mb-1 font-medium" for="borndate">Birth Date</label>
                                    <input type="date" class="input" id="borndate" name="borndate" value="{{ old('borndate', $marketing->borndate ? $marketing->borndate : '') }}" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane hidden" id="contact-content" role="tabpanel" aria-labelledby="contact-tab">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div class="flex flex-col gap-1 md:col-span-2">
                                    ...
                                    <textarea class="textarea" id="address" name="address" rows="2">{{ old('address', $marketing->address) }}</textarea>
                                </div>
                                
                                <div class="flex flex-col gap-1">
                                    <label class="label label-required mb-1 font-medium" for="city">City</label>
                                    <input type="text" class="input" id="city" name="city" value="{{ old('city', $marketing->city) }}" required />
                                </div>
                                
                                <div class="flex flex-col gap-1">
                                    <label class="label label-required mb-1 font-medium" for="phone1">Primary Phone</label>
                                    <input type="text" class="input" id="phone1" name="phone1" value="{{ old('phone1', $marketing->phone1) }}" required />
                                </div>
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div class="mt-2 flex items-center text-sm text-slate-500">
                                            <i class="fas fa-file mr-2 text-primary-500"></i>
                                            <span>Current file: {{ $marketing->ktp }}</span>
                                            <a href="{{ asset('storage/ktp/' . $marketing->ktp) }}" 
                                               target="_blank" 
                                               class="ml-2 text-primary-500 hover:text-primary-600 hover:underline">
                                                View
                                            </a>
                                        </div>
                                        @endif
                        ...
                        <div class="tab-pane hidden" id="financial-content" role="tabpanel" aria-labelledby="financial-tab">
                            ...
                            <div class="mb-6">
                                ...
                                <div id="bankAccountsContainer">
                                    @if($marketing->banks->count() > 0)
                                        @foreach($marketing->banks as $index => $bankAccount)
                                        ...
                                        @endforeach
                                    @else
                                        <!-- Template for when no bank accounts exist -->
                        ...
                        <div class="tab-pane hidden" id="account-content" role="tabpanel" aria-labelledby="account-tab">
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="text-sm">
                                        ...
                                        <p class="text-slate-500 dark:text-slate-400 mt-1">
                                            Update the marketing's account information.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-l-4 border-primary-500 pl-4 py-4">
                                ...
                                @if($marketing->user)
                                <div class="flex flex-col gap-1">
                                    <label class="label mb-1 font-medium">Account Status</label>
                                    <div class="flex items-center mt-1">
                                        <span class="badge {{ $marketing->user->status ? 'badge-soft-success' : 'badge-soft-danger' }}">
                                            {{ $marketing->user->status ? 'Active' : 'Inactive' }}
                                        </span>
                                        <span class="ml-2 text-xs text-slate-500">Last login: 
                                            {{ $marketing->user->last_login_at ? $marketing->user->last_login_at->format('d M Y H:i') : 'Never' }}
                                        </span>
                                    </div>
                                </div>
                                @endif
                    ...
                    <div class="mt-6 flex items-center justify-between border-t border-slate-200 dark:border-slate-700 pt-4">
                        ...
                        <div>
                            ...
                            <button type="submit" class="btn btn-primary ml-2">Update Marketing</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            let bankCounter = {{ max(count($marketing->banks), 1) - 1 }};
            ...
            $('#addBankAccount').on('click', function() {
                ...
                $template.find('input, select').each(function() {
                    ...
                    // If it's a hidden field with an ID, remove it
                    if ($(this).attr('type') === 'hidden' && $(this).attr('name').includes('[id]')) {
                        $(this).remove();
                    } else {
                        // Clear values for other fields
            ...
            $(document).on('click', '.remove-bank-account', function() {
                if ($('.bank-account-record').length > 1) {
                    ...
                    // Check if we need to ensure a default bank
                    if ($('.default-bank-checkbox:checked').length === 0) {
                        $('.default-bank-checkbox').first().prop('checked', true);
                    }
                }
            });
            ...
            function updateDefaultBankLogic() {
                $(document).on('change', '.default-bank-checkbox', function() {
                    if ($(this).prop('checked')) {
                        ...
                        // At least one default must be checked
                        if ($('.default-bank-checkbox:checked').length === 0) {
                            $(this).prop('checked', true);
                            alert('At least one bank account must be marked as default.');
                        }
                    }
                });
            }
