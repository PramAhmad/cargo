            ...
            <div class="card-body p-6">
                ...
                <form action="{{ route('mitra.warehouses.products.update', ['mitra' => $mitra->id, 'warehouse' => $warehouse->id, 'product' => $product->id]) }}" method="POST" id="productForm">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select id="category_product_id" name="category_product_id" class="select">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_product_id', $product->category_product_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" class="input pl-10 price-input" id="mit_price_cbm_display" 
                                       value="{{ number_format(old('mit_price_cbm', $product->mit_price_cbm), 0, ',', '.') }}" 
                                       data-target="mit_price_cbm" />
                                <input type="hidden" name="mit_price_cbm" id="mit_price_cbm" 
                                       value="{{ old('mit_price_cbm', $product->mit_price_cbm) }}" />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" class="input pl-10 price-input" id="mit_price_kg_display" 
                                       value="{{ number_format(old('mit_price_kg', $product->mit_price_kg), 0, ',', '.') }}" 
                                       data-target="mit_price_kg" />
                                <input type="hidden" name="mit_price_kg" id="mit_price_kg" 
                                       value="{{ old('mit_price_kg', $product->mit_price_kg) }}" />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" class="input pl-10 price-input" id="cust_price_cbm_display" 
                                       value="{{ number_format(old('cust_price_cbm', $product->cust_price_cbm), 0, ',', '.') }}" 
                                       data-target="cust_price_cbm" />
                                <input type="hidden" name="cust_price_cbm" id="cust_price_cbm" 
                                       value="{{ old('cust_price_cbm', $product->cust_price_cbm) }}" />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" class="input pl-10 price-input" id="cust_price_kg_display" 
                                       value="{{ number_format(old('cust_price_kg', $product->cust_price_kg), 0, ',', '.') }}" 
                                       data-target="cust_price_kg" />
                                <input type="hidden" name="cust_price_kg" id="cust_price_kg" 
                                       value="{{ old('cust_price_kg', $product->cust_price_kg) }}" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex items-center justify-end border-t pt-4">
                        ...
                        <button type="submit" class="btn btn-primary ml-0">Update Product</button>
                    </div>
                </form>
            </div>
        </div>
