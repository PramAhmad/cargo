<x-app-layout>
    <!-- Page Title Starts -->
    <x-page-title header="Add Warehouse" page="Mitras" />
    <!-- Page Title Ends -->

    <div class="space-y-6">
        <div class="card">
            <div class="card-body p-6">
                <form action="{{ route('mitra.warehouses.store', $mitra->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select id="type" name="type" class="select" required>
                                <option value="sea" {{ old('type') == 'sea' ? 'selected' : '' }}>Sea Freight</option>
                                <option value="air" {{ old('type') == 'air' ? 'selected' : '' }}>Air Freight</option>
                            </select>
                        </div>
                    ...
                    <div class="mt-6 flex items-center justify-end border-t pt-4">
                        ...
                        <button type="submit" class="btn btn-primary ml-2">Save Warehouse</button>
                    </div>
                </form>
            </div>
        </div>
