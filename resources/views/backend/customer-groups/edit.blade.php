<x-app-layout>
    <!-- Page Title Starts -->
    <x-page-title header="Edit Customer Group" page="Customers" />
    <!-- Page Title Ends -->
                ...
                <form action="{{ route('customer-groups.update', $customerGroup->id) }}" method="POST">
                    ...
                    <div class="grid w-full grid-cols-1 gap-4 py-2 md:grid-cols-2">
                        <div class="flex w-full flex-col md:w-auto">
                            <label class="label label-required mb-1 font-medium" for="name">Customer Group Name</label>
                            <input type="text" class="input" id="name" name="name" value="{{ $customerGroup->name }}" />
                        </div>
                    </div>
