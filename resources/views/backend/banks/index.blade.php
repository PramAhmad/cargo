    ...
    <div class="space-y-4">
        ...
        <div class="flex flex-col items-center justify-between gap-y-4 md:flex-row md:gap-y-0">
            ...
            <div class="flex w-full items-center justify-between gap-x-4 md:w-auto">
                ...
                <a class="btn btn-primary" href="{{ route('banks.create') }}" role="button">
                    <i data-feather="plus" height="1rem" width="1rem"></i>
                    <span class="hidden sm:inline-block">Add Bank</span>
                </a>
            </div>
            <!-- Bank Action Ends -->
        </div>
                        ...
                        <td>
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="truncate text-xs text-slate-500 dark:text-slate-400">ID #{{ $bank->id }}</p>
                                </div>
                            </div>
                        </td>
                                    ...
                                    <div class="dropdown-content w-40">
                                        <ul class="dropdown-list">
                                            <li class="dropdown-list-item">
                                                <a href="{{ route('banks.show', $bank->id) }}" class="dropdown-link">
                                                    <i class="h-5 text-slate-400" data-feather="external-link"></i>
                                                    <span>Details</span>
                                                </a>
                                            </li>
                                            <li class="dropdown-list-item">
                                                <a href="{{ route('banks.edit', $bank->id) }}" class="dropdown-link">
                                                    <i class="h-5 text-slate-400" data-feather="edit"></i>
                                                    <span>Edit</span>
                                                </a>
                                            </li>
                                            <li class="dropdown-list-item">
                                                <form action="{{ route('banks.destroy', $bank->id) }}" method="POST" class="delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="dropdown-link w-full text-left" onclick="return confirm('Are you sure you want to delete this bank?')">
        ...
        <!-- Bank Pagination Starts -->
        <div class="flex flex-col items-center justify-between gap-y-4 md:flex-row">
            <p class="text-xs font-normal text-slate-400">
                Showing {{ $banks->firstItem() ?? 0 }} to {{ $banks->lastItem() ?? 0 }} of {{ $banks->total() ?? 0 }} results
            </p>
            
            <!-- Pagination -->
            {{ $banks->links('vendor.pagination.tailwind') }}
        </div>
        <!-- Bank Pagination Ends -->
    </div>
    <!-- Bank List Ends -->
</x-app-layout>