                                ...
                                <td>
                                    <span class="font-normal text-md text-gray-700 dark:text-gray-100 capitalize">
                                        <form action="{{ route('admin.settings.update', $key) }}"
                                            id="settings_form_{{ $key }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            
                                            @if(in_array($key, ['logo', 'fav_icon', 'banner']))
                                                <div class="flex flex-col space-y-2">
                                                    @if($settings)
                                                        <img src="{{ asset('storage/' . $settings) }}" alt="{{ $key }}" class="h-16 w-auto object-contain mb-2">
                                                    @endif
                                                    <input class="input" type="file" name="settings_file" accept="image/*">
                                                    <input type="hidden" name="settings_value" value="{{ $settings }}">
                                                </div>
                                            @else
                        ...
                        <div class="card my-4 overflow-x-auto shadow-sm shadow-gray-300 dark:shadow-gray-700">
                            ...
                            <div class="card-body">
                                <div class="card-content ">
                                    <div class="px-4 py-4">
                                        <form action="{{ route('admin.settings.update', $key) }}"
                                            id="mobile_settings_form_{{ $key }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            @if(in_array($key, ['logo', 'fav_icon', 'banner']))
                                                <div class="flex flex-col space-y-2">
                                                    @if($settings)
                                                        <img src="{{ asset('storage/' . $settings) }}" alt="{{ $key }}" class="h-16 w-auto object-contain mb-2">
                                                    @endif
                                                    <input class="input w-full" type="file" name="settings_file" accept="image/*">
                                                    <input type="hidden" name="settings_value" value="{{ $settings }}">
                                                </div>
                                            @else
                                                <input class="input w-full" type="text" name="settings_value" value="{{ $settings }}">
                                            @endif
                                        </form>
                                    </div>
                                    <p class="card-title flex justify-center mt-5">
                                        <button class="btn btn-primary btn-sm" role="button" 
                                            onclick="document.getElementById('mobile_settings_form_{{ $key }}').submit()">
                                            <i class="w-4" data-feather="upload"></i>
                                            Update
                                        </button>
                                    </p>
