<x-app-layout>
    <div class="container mx-auto px-4 py-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">Edit User: {{ $user->name }}</h1>
                ...
                <form action="{{ route('admin.user.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="input @error('name') is-invalid @enderror">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="input @error('email') is-invalid @enderror">
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" class="input @error('phone') is-invalid @enderror">
                            @error('phone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0 w-20 h-20 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800">
                                    @if($user->images)
                                        <img src="{{ $user->getFirstMediaUrl('images') }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                                    @else
                                ...
                                <div class="flex-grow">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Upload a new profile image (JPEG, PNG, GIF - max 2MB)</p>
                                </div>
                            </div>
                            @error('image')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="md:col-span-2">
                            <div class="bg-gray-50 dark:bg-gray-800 rounded-md p-4 mb-4">
                                <div class="flex items-center justify-between">
                                    <h4 class="text-md font-medium text-gray-700 dark:text-gray-300">Change Password</h4>
                                    <button type="button" id="togglePasswordUpdate" class="text-primary-600 text-sm hover:text-primary-800 focus:outline-none">
                                        Show Password Fields
                                    </button>
                                </div>
                            </div>
                            
                            <div id="passwordFields" class="hidden md:grid md:grid-cols-2 gap-6">
                                <!-- Password -->
                                <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="time_zone" id="time_zone" class="select @error('time_zone') is-invalid @enderror">
                                ...
                                <option value="Asia/Jakarta" {{ old('time_zone', $user->time_zone) == 'Asia/Jakarta' ? 'selected' : '' }}>Asia/Jakarta</option>
                                <option value="America/New_York" {{ old('time_zone', $user->time_zone) == 'America/New_York' ? 'selected' : '' }}>America/New York</option>
                                <option value="Europe/London" {{ old('time_zone', $user->time_zone) == 'Europe/London' ? 'selected' : '' }}>Europe/London</option>
                                <option value="Asia/Tokyo" {{ old('time_zone', $user->time_zone) == 'Asia/Tokyo' ? 'selected' : '' }}>Asia/Tokyo</option>
                            </select>
                            @error('time_zone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            if (toggleButton && passwordFields) {
                toggleButton.addEventListener('click', function() {
                    if (passwordFields.classList.contains('hidden')) {
                        ...
                        toggleButton.textContent = 'Hide Password Fields';
                    } else {
                        passwordFields.classList.add('hidden');
                        toggleButton.textContent = 'Show Password Fields';
                    }
                });
            }
        });
